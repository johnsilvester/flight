...
- (void)resetStickView {
    ...
    if (self.isThrottle) {
        [UIView animateWithDuration:0.3 animations:^{
            self.stick.center = CGPointMake(self.stick.center.x, SIZE/2.f);
        }];
    }else{
        [UIView animateWithDuration:0.3 animations:^{
            self.stick.center = CGPointMake(SIZE/2.f,SIZE/2.f);
...
- (void)updateValues {
    ...
    if ([_delegate respondsToSelector:@selector(didUpdateValuesX:andY:withTag:)]) {
        [_delegate didUpdateValuesX:_xValue andY:_yValue withTag:self.tag];
    }
