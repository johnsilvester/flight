...
final class MSPInterpreter: BluetoothSerialDelegate {
    ...
    func processData(code: Int, data: [UInt8]) {
        ...
        **/
        ...
        case MSP_ALTITUDE: //58
            dataStorage.altitude = Double(getInt32(data, offset: 0))
    ...
    func sendRawRC(channels: [UInt16]) {
        ...
        sendMSP(MSP_SET_RAW_RC, bytes: bytes)
    }
    ...
    func sendMSP(code: Int, bytes: [UInt8]?, callback: (Void -> Void)?) {
        ...
        if code == MSP_SET_RAW_RC { dataStorage.printRCValues()}
